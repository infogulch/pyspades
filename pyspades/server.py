...
class ServerConnection(BaseConnection):
    ...
    def loader_player(self, contained):
        old_team = self.team
    ...
    def loader_orientation_data(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
        x, y, z = contained.x, contained.y, contained.z
        if check_nan(x, y, z):
            self.on_hack_attempt('Invalid orientation data received')
            return
    ...
    def loader_position_data(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
        ...
        if game_mode == CTF_MODE:
            ...
            for entity in self.protocol.entities:
                ...
                if collides and vector_collision(entity, world_object.position):
                    self.check_refill()
    
    def loader_weapon_input(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
    ...
    def loader_input_data(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
    ...
    def loader_weapon_reload(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
    ...
    def loader_hit_packet(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
        ...
        try:
            ...
            return
        valid_hit = world_object.validate_hit(player.world_object, value, 
            HIT_TOLERANCE)
        if not valid_hit:
            return
    ...
    def loader_grenade_packet(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
    ...
    def loader_set_tool(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
    ...
    def loader_set_color(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
    ...
    def loader_block_action(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
        value = contained.value
    ...
    def loader_block_line(self, contained):
        if not self.hp:
            return
        world_object = self.world_object
    ...
    def loader_chat_message(self, contained):
        if not self.name:
            return
        value = contained.value
        if value.startswith('/'):
    ...
    def loader_change_team(self, contained):
        if not self.name:
            return
        team = self.protocol.teams[contained.team]
        ret = self.on_team_join(team)
    ...
    loader_handlers = {
        loaders.ExistingPlayer.id : loader_player,
        loaders.ShortPlayerData.id : loader_player,
        loaders.OrientationData.id : loader_orientation_data,
        loaders.PositionData.id : loader_position_data,
        loaders.WeaponInput.id : loader_weapon_input,
        loaders.InputData.id : loader_input_data,
        loaders.WeaponReload.id : loader_weapon_reload,
        loaders.HitPacket.id : loader_hit_packet,
        loaders.GrenadePacket.id : loader_grenade_packet,
        loaders.SetTool.id : loader_set_tool,
        loaders.SetColor.id : loader_set_color,
        loaders.BlockAction.id : loader_block_action,
        loaders.BlockLine.id : loader_block_line,
        loaders.ChatMessage.id : loader_chat_message,
        loaders.FogColor.id : loader_fog_color,
        loaders.ChangeWeapon.id : loader_change_weapon,
        loaders.ChangeTeam.id : loader_change_team
    }
    ...
    def loader_received(self, loader):
        ...
        handler = self.loader_handlers.get(contained.id, None)
